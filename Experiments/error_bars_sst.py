...
class _SCELoss(nn.Module):
    ...
    def forward(self, probs, labels):
        ...
        for i in range(num_classes):
            ...
            class_sce = Variable(torch.zeros(1)).type_as(softmaxes)
...
def ece_loss(data):
    ...
    return _ECELoss().cuda()(probs, labels).data[0]

def aece_loss(data):
    probs, labels = zip(*data)
    return _AdaptiveECELoss().cuda()(probs, labels).data[0]

def sce_loss(data):
    probs, labels = zip(*data)
    return _SCELoss().cuda()(probs, labels).data[0]
